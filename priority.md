...
### [Low] Inbound exchange doubles charon's single receive-timeout budget (read + handle timed separately) (new — 2026-07-02)
- **Rust:** `crates/priority/src/p2p/handler.rs:206` (`run_inbound` — `timeout(RECEIVE_TIMEOUT, read_request)`) followed by `crates/priority/src/prioritiser.rs:202-217` (`Shared::handle_request` — a *fresh* `tokio::time::sleep(RECEIVE_TIMEOUT)` shared across the enqueue and response-wait selects)
- **Charon ref:** [`p2p/receive.go:56-57`](https://github.com/ObolNetwork/charon/blob/v1.7.1/p2p/receive.go#L56) (`RegisterHandler` stream closure — `s.SetReadDeadline(now+receiveTimeout)` and `ctx, cancel := context.WithTimeout(context.Background(), o.receiveTimeout)`); the same `ctx` is passed into `handleRequest` ([`core/priority/prioritiser.go:224-239`](https://github.com/ObolNetwork/charon/blob/v1.7.1/core/priority/prioritiser.go#L224)), whose enqueue/wait selects use `<-ctx.Done()`.
- **Issue:** Charon creates **one** `receiveTimeout` (default 5s) at stream-handler start; that single deadline bounds the read (via `SetReadDeadline`) **and** the `handleRequest` enqueue + response-wait phase (via the shared `ctx.Done()`) — read + handle together are capped at ~5s. Pluto instead uses **two independent** 5s timers: `run_inbound` wraps only the request read in `timeout(RECEIVE_TIMEOUT, …)`, and `handle_request` then starts a brand-new `sleep(RECEIVE_TIMEOUT)` that begins counting *after* the read completes. So the total time a node holds an inbound priority stream (plus its `oneshot` and `req_buffers` channel slot) is up to ~2×`RECEIVE_TIMEOUT` (~10s) vs charon's ~5s. (The write phase is unbounded in both — charon sets only a read deadline — so that part is not a divergence.)
- **Impact & likelihood:** Resource-holding / mild DoS-amplification parity divergence: a slow-but-authenticated cluster peer (non-cluster peers are gated to the dummy handler and never reach `run_inbound`) that dribbles its request bytes over ~5s and then stalls keeps the inbound exchange alive ~10s on pluto vs ~5s on charon. The *outcome* (no response written on wait timeout) is identical; only the hold duration differs. · only with a misbehaving cluster peer; bounded.
- **PoC:** confirmed by inspection (Go not run). Verified `run_inbound` wraps only `protocol::read_request` in `timeout(RECEIVE_TIMEOUT, …)` (`handler.rs:206`), and `handle_request` starts a fresh `tokio::time::sleep(RECEIVE_TIMEOUT)` used by both the enqueue and response-wait `select!`s (`prioritiser.rs:202-217`), so the two budgets are additive. Charon's single shared-`ctx` deadline reasoned from `receive.go:56-57` + `prioritiser.go:224-239`.
- **Fix:** Give the inbound path a single deadline covering read + handle, matching charon: compute `deadline = Instant::now() + RECEIVE_TIMEOUT` in `run_inbound` and pass the *remaining* budget into `handle_request` (or wrap the whole `run_inbound` body in one `timeout(RECEIVE_TIMEOUT, …)`), rather than starting a fresh `RECEIVE_TIMEOUT` in `handle_request`.
...
**Delta scan (2026-07-02):** added one **[Low]** — the inbound exchange times the request read and the `handle_request` enqueue/wait phases with two independent `RECEIVE_TIMEOUT` timers, so a slow cluster peer can pin an inbound stream ~2× longer than charon's single shared receive-timeout context. Revised counts: 0C/0H/1M/5L/1I.
