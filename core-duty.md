...
Scope: commit 63f6603 "consolidate exp backoff config into pluto_core::expbackoff". New file `crates/core/src/expbackoff.rs` (+41) and its `scheduler.rs` call sites. Verified parity: scheduler `wait_chain_start`/`wait_beacon_sync` use `expbackoff::fast()` (100ms/5s/1.6) on the getter + error paths and `expbackoff::default()` (1s/120s/1.6) on the is-syncing path — byte-identical to Charon's `scheduler.go:735/761` (`FastConfig`) and `:770` (`DefaultConfig`). The old `fast_backoff()`/`default_backoff()` helpers had identical constants, so the scheduler migration is a pure rename with zero behavioral change (the only behavioral change in #505 is in `p2p/bootnode.rs` — see p2p.md).

### [Low] expbackoff: backon jitter diverges from Charon's ±20% and module doc mischaracterizes it
- **Rust:** `crates/core/src/expbackoff.rs:8-12` (module doc), `:28`/`:40` (`.with_jitter()` in `fast`/`default`)
- **Charon ref:** [`app/expbackoff/expbackoff.go:162-169`](https://github.com/ObolNetwork/charon/blob/v1.7.1/app/expbackoff/expbackoff.go#L162) (`Backoff`, `backoff *= 1 + config.Jitter*(randFloat()*2-1)`)
- **Issue:** Charon jitters the computed delay by `±Jitter` (Jitter=0.2) → final delay in `[0.8·d, 1.2·d]`, symmetric around `d`. backon 1.6.0's `with_jitter` does `tmp_cur = tmp_cur + tmp_cur * rng.f32()` (`backoff/exponential.rs:234`, `rng.f32()∈[0,1)`) → final delay in `[d, 2·d)`, **one-sided (always increases), proportional to the CURRENT delay**. Two divergences from Charon: (1) mean delay ≈ 1.5·d vs Charon's ≈ d (pluto retries ~50% slower on average); (2) at the cap, jitter can push delay to ~2× max (≈10s fast / ≈240s default) whereas Charon caps jitter excursion at ~1.2× max. Separately, the new module doc (`:9-10`) claims backon "adds a randomized delay in `(0, base)`" — this is factually wrong: the jitter is proportional to the current (exponentially grown) delay `tmp_cur`, not to `base`/`min_delay`, so it understates the divergence (implies +100ms/+1s when it is really up to +5s/+120s). Behavior is pre-existing at all call sites (the refactor is honest that it introduces "no behavioral change"), but the new canonical module now documents the Charon-parity claim inaccurately.
- **Impact & likelihood:** Retry cadence is systematically longer than Charon and max intervals can double — slower recovery/sync-wait and slower relay re-resolution. No correctness/DoS impact (delays are longer, not tighter; thundering-herd protection is still met). · Always, on every backoff, but effect is only timing.
- **PoC:** n/a (Low — jitter formula confirmed by inspection of backon 1.6.0 `backoff/exponential.rs:234` vs charon `expbackoff.go:162-169`)
- **Fix:** Either accept the divergence and correct the doc to describe backon's actual `[d, 2·d)` jitter (and drop the "(0, base)" claim), or, for true parity, implement a custom backoff that applies `d·(1 + 0.2·(rand·2−1))` (symmetric ±20%) — backon has no built-in symmetric-jitter option, so parity requires a hand-rolled iterator like `relay/dial.rs::backoff_delay`.

### Delta summary
expbackoff consolidation is a faithful, constants-correct refactor (scheduler unchanged). One Low: backon's jitter is one-sided `[d,2d)` proportional to current delay, diverging from Charon's symmetric ±20%, and the new module doc mischaracterizes it as "(0, base)".
