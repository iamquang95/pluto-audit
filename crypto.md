...
### [Medium] Secret material never zeroized (ikm, poly coeffs, recovered secrets, scalars) — ⚠️ PARTIALLY FIXED ([PR #507](https://github.com/NethermindEth/pluto/pull/507))
...
- **PoC:** confirmed by inspection (no PoC needed) — `grep -rn 'zeroize|Zeroizing|ZeroizeOnDrop' crates/crypto/` returns nothing; `PrivateKey = [u8;32]` is `Copy`; the crate's `Cargo.toml` does not depend on `zeroize` (workspace declares `zeroize = "1.8.2"` at Cargo.toml:86 but crypto never pulls it); ~~blst 0.3.16 `SecretKey` has no `Drop`/zeroize (the `serde-secret` zeroize path is feature-gated and unused here)~~ **[CORRECTED — this claim is false; see Correction below]**. Genuinely un-wiped: the `ikm` buffers and the returned `PrivateKey = [u8;32]` (Copy). NOT un-wiped: `poly`/`share_secrets`/`recovered` are `BlstSecretKey`, which *is* zeroize-on-drop.
- **Correction (2026-07-01, PR #507 review):** The struck clause above is **factually wrong**. blst 0.3.16 depends on `zeroize` **non-optionally** (`blst-0.3.16/Cargo.toml:72-74`, pulled with feature `zeroize_derive`), and `min_pk::SecretKey` derives `#[zeroize(drop)]` **unconditionally** inside the `sig_variant_impl!` macro (`blst-0.3.16/src/lib.rs:551-553`, invoked for `min_pk` at `:2199`). The `serde-secret` feature gates only serde *serialization* of secret keys, not the drop-wipe — the two were conflated. Consequence: `poly`, `share_secrets`, and `recovered` (all `BlstSecretKey`; see `blst_impl.rs:128,136`) were **already zeroized on drop before this PR**, so the finding overstated its scope. The only genuinely un-zeroized secret material was the `ikm` buffers and the returned `PrivateKey`/`[u8;32]` outputs.
...
- **Fix status ([PR #507](https://github.com/NethermindEth/pluto/pull/507)):** ⚠️ **Partial.** `ikm` buffers now wrapped in `zeroize::Zeroizing` in `generate_secret_key`/`threshold_split_insecure` ✓; `poly`/`share_secrets`/`recovered` are documented (comments only, no code change) as relying on blst's `#[zeroize(drop)]` — correct, per the Correction above ✓. **Still open:** `PrivateKey`/`SecretKey` remain plain `Copy [u8;32]`, so the returned key bytes (`sk.to_bytes()`, `try_generate_secret_key`, `recover_secret` output) are not zeroize-on-drop — the recommended `Zeroizing<[u8;32]>` newtype for `PrivateKey` is NOT implemented. (Sibling frost-crate scalar zeroization + `Debug` redaction also landed in this PR; those were not tracked as findings here — see frost.md note.)
