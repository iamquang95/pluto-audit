...
**Delta scan (2026-07-02):** added one **[Low]** (CONFIRMED) — ENR TCP/UDP ports < 256 diverge from charon in both directions: pluto encodes them non-minimally (2-byte leading-zero) and its `tcp()`/`udp()` getters rigidly require 2 bytes, so they return `None` for a charon ENR that minimally encodes a port < 256 (silently dropping the peer's dial address in `p2p::bootnode`). Revised counts: 0C / 0H / 2M / 4L / 3I.
...
### [Low] ENR TCP/UDP port encode & decode diverge from charon for ports < 256 (leading-zero encoding + rigid 2-byte decode) (new — 2026-07-02)
- **Rust:** `crates/eth2util/src/enr.rs:124-125` (`EnrEntry::apply`, `port.to_be_bytes()`), `:172-176` (`Record::tcp`), `:181-185` (`Record::udp`)
- **Charon ref:** [`eth2util/enr/enr.go:115-126`](https://github.com/ObolNetwork/charon/blob/v1.7.1/eth2util/enr/enr.go#L115) (`WithTCP`/`WithUDP` → `toBigEndian`), [`enr.go:167-177`](https://github.com/ObolNetwork/charon/blob/v1.7.1/eth2util/enr/enr.go#L167) (`TCP`/`UDP` → `fromBigEndian`), [`enr.go:235-253`](https://github.com/ObolNetwork/charon/blob/v1.7.1/eth2util/enr/enr.go#L235) (`toBigEndian`/`fromBigEndian`)
- **Issue:** charon stores ports as **minimal big-endian** (`toBigEndian` strips leading zero bytes: port 80 → `[0x50]`, one byte) and reads them with `fromBigEndian`, which accepts **any** length. Pluto's `EnrEntry` encodes ports with `u16::to_be_bytes()` — always exactly 2 bytes, keeping the leading zero (port 80 → `[0x00, 0x50]`) — and `Record::tcp()`/`udp()` decode with `value.as_slice().try_into()` into a fixed `[u8; 2]`, which errors (→ `None`) for any value that is not exactly 2 bytes. Two divergences result, both for ports in `1..=255` (high byte zero): (1) **encode** — pluto emits a non-minimal, leading-zero RLP integer, so a pluto ENR for such a port is byte-different from the charon ENR for the same key+port (and would be rejected by strict RLP-integer parsers such as go-ethereum's discv5, which reject leading zeros); (2) **decode** — pluto's `tcp()`/`udp()` return `None` for a charon-produced ENR that minimally encodes a port < 256, silently dropping that address. Note the pluto-internal inconsistency: the sequence-number field in the same `encode_elements` uses `utils::to_big_endian` (`utils.rs`), which *does* strip leading zeros like charon, but the port entries do not. Interop still verifies (both sign/verify over the decoded bytes), so signature checking is unaffected; only the port value round-trip differs.
- **Impact & likelihood:** In `crates/p2p/src/bootnode.rs:306` `multi_addr_from_enr_str` reads `record.udp()`/`record.tcp()`; when a charon relay/bootnode advertises a port < 256 (minimally 1 byte), pluto's getters return `None`, so that transport is skipped, and if **both** ports are < 256 it returns `BootnodeError::EnrNoPort` → pluto cannot derive any dial address for that peer. Conversely a pluto relay/node configured on a port < 256 publishes a non-canonical ENR. · only for ports in `1..=255` — uncommon for libp2p relay/p2p (charon/pluto default is `3610`; privileged ports need root), but reachable with realistic small ports (e.g. 80) and any deliberate small-port config. Escalates to Medium in a mixed charon/pluto cluster that actually uses such a port.
- **PoC:** CONFIRMED — scratch `#[test]` in `crates/eth2util/src/enr.rs` (deleted, worktree clean): (a) encode — `80u16.to_be_bytes() == [0x00, 0x50]` (pluto) vs `utils::to_big_endian(80) == [0x50]` (charon-equivalent); (b) decode — built a charon-style `Record` whose `tcp`/`udp` kvs are the minimal `[0x50]` (port 80), signed it via `sign(&key, encode_elements(&[], &kvs))`, serialized, and re-parsed through the public `Record::try_from` — the signature **verified** yet `parsed.tcp() == None` and `parsed.udp() == None` (observed output `tcp() = None`, `udp() = None`), i.e. the port is silently dropped exactly as described. Charon `toBigEndian`/`fromBigEndian` variable-length behavior confirmed verbatim at `enr.go:235-253` (Go not run).
- **Fix:** Encode ports minimally to match charon (`utils::to_big_endian(port as usize)` in `EnrEntry::apply`, mirroring the seq-number field) and decode with a length-tolerant big-endian read (accept 0..=2 bytes, left-padding) instead of a fixed `[u8; 2]` `try_into`. Same applies to `ip()` if variable-length IPs ever arise (currently IPv4-only → always 4 bytes, unaffected).
